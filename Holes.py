...
class Holes:
        ...
        def ccnext(self,k,orient ):
            if orient == 1:
                ...
                x0,x1,y0,y1=self.conf.getConPos2(self.Ibonds[k],particle)
            else:
                particle = self.Ibonds[k]
                # angle of the original bond (signed), and between -pi to pi
                x0,x1,y0,y1=self.conf.getConPos2(self.Jbonds[k],particle)
            ...
            for l in nnei:
                ...
                if self.Ibonds[l]==particle:
                    x0,x1,y0,y1=self.conf.getConPos2(particle,self.Jbonds[l])
                    lr0 = 1
                elif self.Jbonds[l]==particle:
                    x0,x1,y0,y1=self.conf.getConPos2(particle,self.Ibonds[l])
                    lr0 = -1
                else:
                    print "Problem!!"
        ...
        def sortHoles(self):
            ...
            return self.nHole,self.HolePerim,self.HoleArea
                ...
                if self.isHole[f]:
                    ...
                    while not done:
                        ...
                        if bend ==bstart:
                            ...
                            #done = True
                        c+=1
                    newface = ptch.Polygon(np.transpose(np.array([polyx, polyy])),edgecolor=(rgbpattern[f,0],rgbpattern[f,1],rgbpattern[f,2]), linewidth=3, facecolor=(rgbpattern[f,0],rgbpattern[f,1],rgbpattern[f,2]),alpha=0.5)
                    axval.add_patch(newface)
