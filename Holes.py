...
class Holes:
    ...
    def ccnext(self,k,orient ):
        if orient == 1:
            ...
            x0,x1,y0,y1=self.conf.getConPos2(self.Ibonds[k],particle,True)
        else:
            particle = self.Ibonds[k]
            # angle of the original bond (signed), and between -pi to pi
            x0,x1,y0,y1=self.conf.getConPos2(self.Jbonds[k],particle,True)
        ...
        for l in nnei:
            ...
            if self.Ibonds[l]==particle:
                x0,x1,y0,y1=self.conf.getConPos2(particle,self.Jbonds[l],True)
                lr0 = 1
            elif self.Jbonds[l]==particle:
                x0,x1,y0,y1=self.conf.getConPos2(particle,self.Ibonds[l],True)
                lr0 = -1
            else:
                print("Problem!!")
    ...
    def sortHoles(self):
        ...
        for f in range(self.nface):
            ...
            polyy=[]
            while not done:
                x0,x1,y0,y1=self.conf.getConPos2(self.I[bend],self.J[bend],True)
                polyx.append(x0)
                polyy.append(y0)
                bend = self.Next[bend]
                if bend ==bstart:
