...
# ====== Color helper function, essentially give jet color map in a convenient form ====
    ...
    def plotStresses(self,plotCir,plotVel,plotCon,plotF,plotStress,**kwargs):
        ...
        if (plotF):
            ...
            fval_max=np.sqrt(np.amax(self.conf.fnor)/fscale)
            for k in range(len(self.conf.I)):
                fval=np.sqrt(self.conf.fnor[k]/fscale)
                x0,x1,y0,y1=self.conf.getConPos(k)
                #conlink=lne.Line2D([x0,x1],[y0,y1],color=Fcolor(fval),lw=2*fval)
                conlink=lne.Line2D([x0,x1],[y0,y1],color=(0.9,fval*155.0/255.0/fval_max,0),lw=2*fval)
                axval.add_line(conlink)
#                if self.verbose:
#                    if (k>=self.ncon):
#                        axval.text(x0+0.5*(x1-x0)-self.small*np.sin(ang),y0+0.5*(y1-y0)+self.small*np.cos(ang),str(k),fontsize=8)
...
#                        axval.text(x0+0.5*(x1-x0),y0+0.5*(y1-y0),str(k),fontsize=8)    
        ...
        #axval.set_title('Forces and contacts')
    ...
    def plotPebbles(self,plotCir,plotPeb,plotPebCon,plotClus,plotOver,**kwargs):
            ...
            if (plotPeb):
                for j in range(3):
                    if(self.pebbles.pebbles[k,j]==-1):
                        ...
                        if(t==3):
                            color='b'
                        cir2=ptch.Circle((self.conf.x[k],self.conf.y[k]),radius=0.5*self.conf.rad[k],ec=color,fc=color,linewidth=2)
        ...
        if (plotClus):
            ...
            rgbpattern[0,:]=np.array([20.0/255.0,20.0/255.0,235.0/255.0])
            ...
            for k in range(self.ncon):
                ...
                if (self.pebbles.cluster[k]!=-1):
                    if (k>=self.ncon):
                        ...
                        conlink=lne.Line2D([x0-self.small*np.sin(ang),x1-self.small*np.sin(ang)],[y0+self.small*np.cos(ang),y1+self.small*np.cos(ang)],linewidth=2,color=(rgbclus[k,0],rgbclus[k,1],rgbclus[k,2]))
                    else:
                        conlink=lne.Line2D([x0,x1],[y0,y1],linewidth=2,color=(rgbclus[k,0],rgbclus[k,1],rgbclus[k,2]))
                    axval.add_line(conlink)
                else:
                    if (k>=self.ncon):
                        ang=np.arctan((y1-y0)/(x1-x0))
        ...
        if self.conf.datatype=='simulation':
            ...
            axval.set_xlim(xmin,xmin+self.Lx+120)
            axval.set_ylim(ymin,ymin+self.Ly+120)
            # axval.set_xlim(xmin+0.5*self.Lx+320,xmin+self.Lx-20)
            # axval.set_ylim(ymin+0.5*self.Ly+60,ymin+self.Ly-280)
        #axval.set_title(str(self.pebbles.freepeb) + ' free pebbles, ' + str(self.pebbles.fail) + ' failed contacts')
        # returning figure pointer for use outside (like plotting or saving)
    ...
    def ModeClusters(self,mode,thresh=2e-5):
        ...
        # print "thresh is changed from ", thresh, " to ", 0.25*np.mean(eign+eigt) 
        ...
        for k in range(self.ncon):         
            ...
            if mode == 'translations':
                if (eign[k]+eigt[k])<thresh:
                        if eigr[k]>thresh:
                            conlink=lne.Line2D([x0,x1],[y0,y1],color=(0.65,0.65,0.65),lw=2.0)
                        else:
                            conlink=lne.Line2D([x0,x1],[y0,y1],color=(127.0/255.0,0,1.0),lw=2.0)
                else:
                    conlink=lne.Line2D([x0,x1],[y0,y1],color=(0.65,0.65,0.65),lw=2.0)
                axval.add_line(conlink)
...
#                       axval.add_patch(cir1)
        ...
        if (plotdis):
            ...
            # axval.hist(np.log(D2_min),bins=40)
            # use now a color scheme not unlike the ones above.
            # This has a rather odd distribution ... 
            D2_scale=np.mean(np.sqrt(D2_min))-np.amin(np.sqrt(D2_min))
            radscale=np.mean(self.conf.rad)
            Fcolor,Fmap=self.color_init(3.0)
            for k in range(self.N):
                # duse=np.sqrt(D2_min[k])-np.amin(np.sqrt(D2_min))
                # if (duse>2.999*D2_scale):
                #     duse=2.999*D2_scale
                ...
                #cir1=ptch.Circle((self.conf.x[k],self.conf.y[k]),radius=0.25*radscale*duse/D2_scale,ec=(1-duse/D2_scale/3.0,1-duse/D2_scale/3.0,1-duse/D2_scale/3.0),fc=(1-duse/D2_scale/3.0,1-duse/D2_scale/3.0,1-duse/D2_scale/3.0), linewidth=2)
                cir1=ptch.Circle((self.conf.x[k],self.conf.y[k]),radius=0.25*radscale*duse,ec=(greyscale,greyscale,greyscale),fc=(greyscale,greyscale,greyscale), linewidth=2)
                axval.add_patch(cir1)
...
#                               axval.add_line(conlink)
        ...
        plt.title(r'$ D^2_{min} $ (log scale) ')
        return fig
