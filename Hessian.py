...
class Hessian:
	...
	#=========================== Hessian calculation ===============================
	def makeHessian(self,frictional,recomputeFnor,stabilise=1e-8,verbose=False):
		...
		for k in range(self.ncon):
			...
			# once for contact ij
			self.Hessian[3*i:(3*i+3),3*j:(3*j+3)]=Hij
			...
			# And put it into the Hessian
			# now for contact ji
			self.Hessian[3*j:(3*j+3),3*i:(3*i+3)]=Hji
			...
			self.Hessian[3*i:(3*i+3),3*i:(3*i+3)]+=Hijdiag
			...
			# And then *add* it to the diagnual
			self.Hessian[3*j:(3*j+3),3*j:(3*j+3)]+=Hjidiag
			


                # add a very small, diagonal bit to stabilise zero modes
                self.Hessian[range(3*self.N),range(3*self.N)]+=stabilise
                favx/=self.ncon
                favy/=self.ncon
                frotav/=self.ncon
	...
	def ModeThresh(self,thresh):	
		...
		eigy/=self.nzero
		eigr/=self.nzero
		self.isrigid=[index for index,value in enumerate(eigx+eigy+eigr) if value<thresh]
