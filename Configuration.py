...
class Configuration:
    ...
    def getD2min(self,threshold_rad):
        ...
        self.C8=0.0
        def nested_f(x):
            return self.C1*(x[0]**2+x[2]**2)+self.C2*(x[1]**2+x[3]**2)+self.C3*(x[0]*x[1]+x[2]*x[3])+self.C4*x[0]+self.C5*x[1]+self.C6*x[2]+self.C7*x[3]+self.C8
        for k in range(len(self.x)):
            ...
            if len(temp[0])>0:
                ...
                for neighbor in temp[0]:
                    self.C1+=(self.x[neighbor]**2)
                    self.C2+=(self.y[neighbor]**2)
                    self.C3+=(2*self.x[neighbor]*self.y[neighbor])
                    self.C4+=(-2*self.xnext[neighbor]*self.x[neighbor])
                    self.C5+=(-2*self.xnext[neighbor]*self.y[neighbor])
                    self.C6+=(-2*self.ynext[neighbor]*self.x[neighbor])
                    self.C7+=(-2*self.ynext[neighbor]*self.y[neighbor])
                    self.C8+=(self.xnext[neighbor]**2+self.ynext[neighbor]**2)
            minimum = fmin(nested_f,np.asarray([1,1,1,1]),disp=False)
    ...
    def Disp2Contacts(self,minThresh,debug=False):
        ...
        thresh=np.mean(disp2n+disp2t)+np.std(disp2n+disp2t)
