...
class ConfigurationExpAnnulus(Configuration):
    ...
    def ReadData(self, posfile,confile,step,verbose=False):    
        #posfile = self.folder +'/particle_positions.txt'
        print(posfile)
        self.isPosdata=True
        try:
            coords=np.loadtxt(posfile, delimiter=',')
        ...
        self.isCondata=True
        try:
            condata=np.loadtxt(confile, delimiter=',')
...
class ConfigurationExpSquare(Configuration):
    ...
    def __init__(self,param):
        ...
        super(ConfigurationExpAnnulus,self).__init__('experiment_square')
...
class ConfigurationSimulation(Configuration):
    ...
    def __init__(self,param):
        ...
        self.getParameters(param["folder"])
        self.folder = param["folder"]
        #self.setup = param["setup"] # redundant as needs to be same 'experiment_annulus' as in runscript
        ...
        try:
            self.distSnapshot=param['distSnapshot']
        except:
