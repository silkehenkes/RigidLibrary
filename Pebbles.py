    ...
    def rigid_cluster(self):
        ...
        for k in range(self.ncon2):
            ...
            if (self.cluster[k] == -1 and self.ptr[k] == 0):
                ...
                i = int(self.Ifull[k])
                j = int(self.Jfull[k])
                bonds = [val for val in self.conmat[i]
                         if val in self.conmat[j]]
                stopthis = False
                if len(bonds) > 1:
                    if self.verbose:
                    ...
                    if self.cluster[k2] != -1:
                        ...
                        print("Double bond " + str(k) + " neighbour of " + str(k2) +
                              " was also labeled cluster " + str(self.cluster[k]))
                ...
                if stopthis == False:
                    ...
                    for l in range(self.game2):
                        result = self.enlarge_cover(pebblescopy, i, j)
                        if result == 1:
                            print("Error: no " + str(self.game2) +
                                  "global DOF pebbles in rigidity search at contact" + str(k))
                    ...
                    if (cluslen > 0):
                        ...
                        if ctry != self.cidx:
                            print(
                                "WARNING: Found a (part) duplicate cluster. Merge length here with cluster " + str(ctry))
                            cidxstore = self.cidx
                            self.cidx = ctry
                    ...
                    if (marked[u] and rigid[u]):
                        ...
                        if self.verbose:
                            print('Added bond ' + str(c) +
                                  ' to cluster ' + str(self.cidx))
                if self.verbose:
                    print('rigid')
                self.pcluster[i].append(self.cidx)
                if self.verbose:
                    print('Added particle ' + str(i) +
                          ' to cluster ' + str(self.cidx))
        return self.cidx
    ...
    def rearrange_pebbles(self, pbcopy, i, j, path):
        ...
        if (path[i] == -1):
            ...
            while (path[i] != -1):
                ...
                if (l == pbcopy[i, 0]):
                    # remove pebble and replace on ij contact, i.e. change label from l to i
                    pbcopy[i, 0] = j
                else:
                    if (l == pbcopy[i, 1]):
                        pbcopy[i, 1] = j
